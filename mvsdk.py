...
def CameraGigeEnumerateDevice(ipList=None, MaxCount = 32):
	"""
	ipList: Optional list/tuple/iterable of interface IP strings to scan.
	        If omitted or empty, the SDK will probe all adapters.
	"""
	if ipList is None:
		ipList = []
	if type(ipList) in (list, tuple):
		# Force eager conversion to avoid Python 3 map iterator len issues
		ipList = [_str_to_string_buffer(x) for x in ipList]
	else:
